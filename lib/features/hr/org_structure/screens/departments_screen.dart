...
class _DepartmentsDashboardState extends State<DepartmentsDashboard> {
  // الوضع الافتراضي: قائمة (false) حسب طلبك السابق
  bool isKanban = false; 
    ...
    setState(() {
      departments = rawDepts.map((item) {
        ...
        return Department(
          ...
          parentDept: item.metaData?['type'] ?? 'GEN',
        );
      }).toList();
    });
  }
      ...
      actions: [
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              _buildViewToggleButton(Icons.grid_view, true, "كانبان"),
              _buildViewToggleButton(Icons.list, false, "قائمة"),
            ],
          ),
        )
      ],
  ...
  Widget _buildViewToggleButton(IconData icon, bool isKanbanMode, String tooltip) {
    final isSelected = isKanban == isKanbanMode;
    return InkWell(
      onTap: () => setState(() => isKanban = isKanbanMode),
      child: Container(
  ...
  // --- التعديل هنا: جعل المربعات أصغر ومتجاوبة ---
  Widget _buildKanbanView() {
    return LayoutBuilder(
      builder: (context, constraints) {
        // تحديد عدد الأعمدة بناءً على عرض الشاشة لجعل البطاقات أصغر
        int crossAxisCount = 2;
        if (constraints.maxWidth > 1400) {
          crossAxisCount = 5; // شاشات كبيرة جداً: 5 بطاقات
        } else if (constraints.maxWidth > 1100) {
          crossAxisCount = 4; // شاشات لابتوب: 4 بطاقات
        } else if (constraints.maxWidth > 700) {
          crossAxisCount = 3; // تابلت: 3 بطاقات
        }
        // موبايل: 2 بطاقة
        ...
        return GridView.builder(
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
            ...
            childAspectRatio: 1.3, // نسبة العرض للارتفاع (كلما زاد الرقم قصر الطول)
            crossAxisSpacing: 10,  // تقليل المسافات البينية
            mainAxisSpacing: 10,
          ),
...
class DepartmentDetailsScreen extends StatelessWidget {
  ...
  Widget _buildFinancialTab() {
    return ListView(
      padding: const EdgeInsets.only(top: 16),
      children: [
        _buildReadOnlyField("مركز التكلفة", "CC-${department.id.substring(4)}"), 
...
class _DepartmentCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.all(10), // حشوة أصغر
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            // العنوان بحجم خط أصغر قليلاً
            Text(
              department.name, 
              maxLines: 2, 
            ...
            Row(
              children: [
                ...
                Expanded(child: Text(department.managerName, style: const TextStyle(fontSize: 10, color: Colors.grey), overflow: TextOverflow.ellipsis)),
              ],
            ),
  ...
  Widget _buildBadge(String text) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 2), // حشوة أصغر
      decoration: BoxDecoration(
        color: Colors.grey[100],
        borderRadius: BorderRadius.circular(4),
      ),
      child: Text(text, style: const TextStyle(fontSize: 9)), // خط أصغر
    );
  }
}